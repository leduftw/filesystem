...
DWORD WINAPI nit2run() {
	...
	{
		wait(mutex);
		cout << threadName << ": Broj fajlova na disku je " << FS::readRootDir() << endl;  // trebalo bi da bude 3 (fajl1.dat, fajl2.dat i fajll5.dat)
		signal(mutex);
	}
...

	{
		...
		wait(mutex);
		cout << threadName << ": Otvoren fajl '" << filepath << "' i pozicionirani smo na pocetku" << endl;
		signal(mutex);
		...
		wait(mutex);
		cout << threadName << ": Prepisana prva polovina '" << filepath << "' u '" << filepath1 << "'" << endl;
		signal(mutex);
